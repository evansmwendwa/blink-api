...
class Embed 
{
    ...
    public function parse( $content ) {
        ...
        // this is the original code used in wordpress
        //return preg_replace_callback( '|^\s*(https?://[^\s"]+)\s*$|im', array( $this, 'autoembed_callback' ), $content );

        $regex		= '/{embed\s(.*?)}/i';
        return preg_replace_callback( $regex, array( $this, 'autoembed_callback' ), $content );
    }
            ...
            switch ( $data->type ) {
                    ...
                    case 'rich':
                            if ( ! empty( $data->html ) && is_string( $data->html ) )
                                    $return = '<div class="embed-container video-container embed-'.$data->type.'">'.$data->html.'</div>';
                            break;
                    case 'link':
